...
The `CdbIterator` allows iterating over all key-value pairs in the database.

```ascii
User Calls:
let mut iter = cdb.iter()?;
while let Some(item_result) = iter.next_item()? {
    // process item (key, value)
}
...
1. `Cdb::iter()`:
   ...
   - `current_data_offset` is initialized to the start of the data records section (i.e., `HEADER_SIZE = 4096`).
   - `end_of_data_offset` is determined by finding the minimum `table_pos` from the header. This marks where the first hash table begins, and thus where data records end. If all hash tables are empty, it might be more complex or rely on total file size. A simpler way is to use the start of the first hash table as recorded in the header.

2. `CdbIterator::next_item()`:
   a. Check Bounds:
      - If `current_data_offset >= end_of_data_offset`, iteration is complete. Return `Ok(None)`.
...
Key points:
*   Iteration reads data records sequentially from disk.
*   It needs to know where the data records section ends. This is typically the start of the first hash table. The `CdbWriter` records this information (or it can be inferred from the header).
