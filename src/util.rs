...
pub fn read_tuple<R: ReaderAt + ?Sized>(reader: &R, offset: u64) -> Result<(u64, u64)> {
    ...
    // Safely convert parts of the buffer to u64.
    // These try_into calls should not fail if read_exact_at succeeded with a 16-byte buffer.
...
mod tests {
    ...
    fn test_read_tuple_success() {
        ...
        let val2: u64 = 0x090A0B0C0D0E0F10;
        ...
        // Test with offset
        let val3: u64 = 0x1112131415161718;
        let val4: u64 = 0x191A1B1C1D1E1F20;
    ...
    fn test_write_tuple_success() {
        let val1: u64 = 0xA1A2A3A4A5A6A7A8;
        let val2: u64 = 0xB1B2B3B4B5B6B7B8;
        let mut buffer = Cursor::new(Vec::new());

        write_tuple(&mut buffer, val1, val2).unwrap();

        let written_bytes = buffer.into_inner();
        assert_eq!(written_bytes.len(), 16);
