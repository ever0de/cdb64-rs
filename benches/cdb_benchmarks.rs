...
const NUM_ENTRIES_FOR_BENCH: usize = 10000; // Number of entries for benchmark
    ...
    group.bench_function("write_temp_file", |b| {
        b.iter(|| {
            ...
            for (key, value) in data.iter() {
                writer.put(black_box(key), black_box(value)).unwrap();
            }
...
fn cdb_read_benchmark(c: &mut Criterion) {
        ...
        b.iter_batched(
            ...
            |cdb| {
                for key in keys_to_lookup.iter() {
                    if let Some(value_bytes) = cdb.get(black_box(key)).unwrap() {
...

    {
        ...
        group.bench_function("get_from_file_cached", |b| {
            b.iter(|| {
                for key in keys_to_lookup.iter() {
                    if let Some(value_bytes) = cdb_file_cached.get(black_box(key)).unwrap() {
                        black_box(value_bytes);
                    }
                }
            })
    ...
    group.bench_function("get_from_memory", |b| {
        b.iter(|| {
            for key in keys_to_lookup.iter() {
                if let Some(value_bytes) = cdb_in_memory.get(black_box(key)).unwrap() {
                    black_box(value_bytes);
                }
            }
        })
